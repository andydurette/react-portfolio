export const portfolioData = [
  {
    "id": 1,
    "imageUrl": "content/img/portfolio/QaRds.jpg",
    "altText": "QaRds App business cards for a digital future",
    "title": "QaRds",
    "name": "QaRds",
    "description": "Application built in my University of Toronto Fullstack course, it creates user accounts and digital business cards for the users.",
    "siteUrl": "https://business-qard.herokuapp.com/",
    "siteAria": "View the QaRds portfolio website",
    "repoUrl": "https://business-qard.herokuapp.com/",
    "repoAria": "View the QaRds github repository",
  },
  {
    "id": 2,
    "imageUrl": "content/img/portfolio/toronto_safety.jpg",
    "altText": "Toronto Neighbourhood Safety app",
    "title": "Is My Neighbourhood Safe",
    "name": "Is My Neighbourhood Safe",
    "description": "Application built in my University of Toronto Fullstack course, it compares crime data by neighbourhood for all of toronto.",
    "siteUrl": "https://andydurette.github.io/UFT-Project-1/",
    "siteAria": "View Toronto Neighbourhood safety portfolio website",
    "repoUrl": "https://github.com/andydurette/UFT-Project-1",
    "repoAria": "View Toronto Neighbourhood safety github repository",
  },
  {
    "id": 3,
    "imageUrl": "content/img/portfolio/weather-2.jpg",
    "altText": "Toronto weather json data api site",
    "title": "React.js Weather App",
    "name": "React.js Weather App",
    "description": "A personal project where I worked with openweather API, React.js, and Netlify to host a weather application.",
    "siteUrl": "https://openweather-react.netlify.com/",
    "siteAria": "View React.js Weather App portfolio website",
    "repoUrl": "https://github.com/andydurette/openweather-react",
    "repoAria": "View React.js Weather App portfolio repo",
  },
  {
    "id": 4,
    "imageUrl": "content/img/portfolio/fitness-tracker.jpg",
    "altText": "code quiz",
    "title": "code quiz",
    "name": "Fitness Tracker",
    "description": "Application built in my University of Toronto Fullstack course, using mongo as a database and node for handling crud operations.",
    "siteUrl": "https://workout-tracking.herokuapp.com",
    "siteAria": "View code quiz website",
    "repoUrl": "https://github.com/andydurette/Workout-Tracker",
    "repoAria": "View code quiz github repository",
  },
  {
    "id": 5,
    "imageUrl": "content/img/portfolio/burger.jpg",
    "altText": "Eat the burger app",
    "title": "Eat the burger",
    "name": "Eat the burger",
    "description": "Application built in my University of Toronto Fullstack course, an mvc app handling rest operators in a burger ordering app.",
    "siteUrl": "https://eat-all-da-burger.herokuapp.com",
    "siteAria": "View Eat The Burger portfolio website",
    "repoUrl": "hhttps://github.com/andydurette/MVC-Eat-Da-Burger",
    "repoAria": "View Eat The Burger github repository",
  },
  {
    "id": 6,
    "imageUrl": "content/img/portfolio/note.jpg",
    "altText": "Toronto Neighbourhood Safety app",
    "title": "Express Note Taker",
    "name": "Express Note Taker",
    "description": "Application built in my University of Toronto Fullstack course, a note taking app where I built a express backend to handle data.",
    "siteUrl": "https://express-note-taker.herokuapp.com",
    "siteAria": "View Express Note Taker portfolio website",
    "repoUrl": "https://github.com/andydurette/Express-Note-Taker",
    "repoAria": "View Express Note Taker github repository",
  },
  {
    "id": 7,
    "imageUrl": "content/img/portfolio/pass-gen.jpg",
    "altText": "Password Generator app",
    "title": "Password Generator app",
    "name": "Password Generator",
    "description": "Small Application where you can generate a password based off you own choice of requirements to be met.",
    "siteUrl": "https://andydurette.github.io/password-generator",
    "siteAria": "View password generator website",
    "repoUrl": "https://github.com/andydurette/password-generator",
    "repoAria": "View password-generator github repository",
  },
  {
    "id": 8,
    "imageUrl": "content/img/portfolio/30-day-challenge.jpg",
    "altText": "30 day icon challenge",
    "title": "Password Generator app",
    "name": "30 day icon challenge",
    "description": "A personal project where I  designed an icon a day for a month to better produce vector graphics quickly and efficently.",
    "siteUrl": "https://icon-gallery.netlify.com/",
    "siteAria": "View 30 Day Icon Challenge portfolio website",
    "repoUrl": "https://github.com/andydurette/icon-gallery",
    "repoAria": "View 30 Day Icon Challenge portfolio repo",
  },
  {
    "id":9,
    "imageUrl": "content/img/portfolio/bee.jpg",
    "altText": "Bees of America",
    "title": "Bees of America",
    "name": "Bees of America",
    "description": "A personal project where I researched the history of bees and used React.js, github and Netlify to host a react routed app.",
    "siteUrl": "https://bees-react.netlify.com/",
    "siteAria": "View Bees of America portfolio website",
    "repoUrl": "https://github.com/andydurette/Bees",
    "repoAria": "View Bees of America portfolio website",
  }
]




